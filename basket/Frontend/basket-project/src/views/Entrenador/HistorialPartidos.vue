        ...
        <div class="flex-1 overflow-y-auto pr-2 pb-10">
            ...
            <div v-else class="space-y-8">
                <div v-for="p in partidos" :key="p.id_partido" class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="bg-slate-900 px-6 py-4 flex flex-col md:flex-row justify-between md:items-center text-white">
                        <div class="flex items-center text-sm font-bold text-gray-300 mb-4 md:mb-0">
                    ...
                    <div class="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8 bg-gray-50">
                        ...
                        <div class="space-y-4">
                            <h4 class="font-black text-gray-800 uppercase tracking-widest text-xs border-b border-gray-300 pb-2">Reporte Oficial del Partido</h4>
                        ...
                        <div class="space-y-4">
                            <h4 class="font-black text-indigo-900 uppercase tracking-widest text-xs border-b border-indigo-200 pb-2">Evaluación del Arbitraje</h4>
                            
                            <div v-if="!p.id_evaluacion" class="bg-indigo-50 p-6 rounded-xl border border-indigo-100 text-center">
                                ...
                                <button @click="abrirModalEvaluacion(p)" class="w-full py-2.5 bg-indigo-600 text-white font-bold rounded-lg shadow-sm hover:bg-indigo-700 transition">
                                    Evaluar al Árbitro
                                </button>
                            </div>
...
import { obtenerEquipoDeEntrenadorService } from '../../services/equiposService' // Importamos para obtener el ID de tu equipo

const idEntrenador = localStorage.getItem('usuario_id')
const miEquipoId = ref(null) // CORRECCIÓN 3: Declaramos la variable
const partidos = ref([])
...
const cargarHistorial = async () => {
    try {
        // CORRECCIÓN 1: Solo un parámetro
